...
class Calculator_App(tkinter.Frame):
    ...
    def add_components(self):
        ...
        for button in buttons:
            varColumn +=1
            self.btnClick = ClickButtons(self, value=button, width=4, height=3, padx=16, pady=16, bd=8, text=button.replace(' ',''), relief="raised") 
            if button == 'C':
...
def Functional_Output(Validate_Input):
    try:
        while '(' in Validate_Input and ')' in Validate_Input:
            ...
            receive_input = receive_value.replace('(','').replace(')','')
